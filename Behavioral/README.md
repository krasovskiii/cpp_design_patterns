...
| 模式 | 优先级 | 一句话 |
|------|:---:|--------|
| [Strategy](#strategy--t1) | `[T1]` | 运行时/编译期选择系统行为（定价模型切换、执行算法） |
| [Observer](#observer--t1) | `[T1]` | 发布-订阅（行情推送、实时风控告警） |
| [Command](#command--t2) | `[T2]` | 将操作封装为对象（订单指令、OMS 撤单改单） |
| [State](#state--t2) | `[T2]` | 用有限状态机控制行为（订单状态机） |
| [Template Method](#template-method--t2) | `[T2]` | 定义算法骨架，子类实现细节（回测框架） |
| [Chain of Responsibility](#chain-of-responsibility--t3) | `[T3]` | 责任链依次处理（订单风控校验链路） |
| [Visitor](#visitor--t3) | `[T3]` | 遍历结构而不修改层级（计算多种 Greeks） |
| [Interpreter](#interpreter--t4) | `[T4]` | 解析结构化文本（交易规则 DSL、协议解析） |
| [Iterator](#iterator--t4) | `[T4]` | 遍历数据结构（时间序列/持仓遍历） |
| [Mediator](#mediator--t4) | `[T4]` | 集中式通信（中央订单路由、撮合引擎） |
| [Memento](#memento--t4) | `[T4]` | 保存状态快照用于恢复（情景回溯） |


## Chain of Responsibility — `[T3]`
...
- [functional strategy](strategy/strategy_functional.cpp): 现代 C++ 用 `std::function` 替代策略类层级，零继承零虚函数，呼应「避免过度设计」章节（含定价模型切换案例）。
